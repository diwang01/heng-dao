...
public class TransactionInterceptorExtra extends TransactionInterceptor implements MethodInterceptor, Serializable {
     ...
     */
    public TransactionInterceptorExtra() {
    }
     ...
     * @see #setTransactionAttributes(java.util.Properties)
     */
    public TransactionInterceptorExtra(PlatformTransactionManager ptm, Properties attributes) {
     ...
     * @see #setTransactionAttributeSource(TransactionAttributeSource)
     */
    public TransactionInterceptorExtra(PlatformTransactionManager ptm, TransactionAttributeSource tas) {
        super(ptm, tas);
    }
    ...
    private void holdTransactionDef(PlatformTransactionManager tm, TransactionAttribute txAttr,
        ...
        TransactionHolder.setTransactionInfo(new TransactionWrapper(tm, txAttr, joinpointIdentification));
    }
    ...
    private void commitTransactionAfterReturning() {
        ...
        if (txTree != null) {
            for (Map.Entry<DataSource, LinkedList<TransactionWrapper>> entry : txTree.entrySet()) {
                Iterator<TransactionWrapper> itor = entry.getValue().descendingIterator();
                //
                while (itor.hasNext()) {
                    TransactionWrapper txInfo = itor.next();
                    // commit
     ...
     * @return void
     */
    private void prepareCommitOrRollback(TransactionWrapper txInfo) {
