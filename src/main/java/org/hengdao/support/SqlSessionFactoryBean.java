 ...
 * @author  barney.wang
 */
public class SqlSessionFactoryBean  implements ApplicationContextAware, MultiDataSourceIF {
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        if (getShardDataSources() != null && getShardDataSources().size() > 0) {
            ...
            for (Map.Entry<String, DataSource> entry : getShardDataSources().entrySet()) {
    ...
    private SqlSessionFactory buildSqlSessionFactory(DataSource dataSource) throws IOException {
        ShardIntercept shardIntercept = new ShardIntercept();
        shardIntercept.setSqlConverter(sqlConverter);

        Configuration configuration = null;
        SpringManagedTransactionFactory transactionFactory = null;
        //
        configuration = new Configuration();
        configuration.addInterceptor(shardIntercept);
        //
